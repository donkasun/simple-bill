...
const StyledTextarea: React.FC<StyledTextareaProps> = ({ label, style, error, required, id, ...props }) => {
  ...
  const errorId = useMemo(() => `textarea-error-${Math.random().toString(36).slice(2)}`, []);
    ...
    <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      <div ref={wrapperRef} style={{ position: 'relative', ...style }}>
        ...
        <textarea
          ...
          required={required}
          {...props}
          style={{ position: 'relative', zIndex: 1, width: 'calc(100% - 28px)', padding: '10px 12px', border: 'none', outline: 'none', background: 'transparent', resize: 'vertical' }}
        />
      </div>
      {error && (
